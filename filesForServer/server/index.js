...
const PLAID_CLIENT_ID =
  process.env.PLAID_CLIENT_ID || "<REPLACE_WITH_PLAID_CLIENT_ID>"; // <-- replace or set env
...
if (!PLAID_CLIENT_ID || PLAID_CLIENT_ID.startsWith("<REPLACE")) {
  console.warn(
    "Warning: PLAID_CLIENT_ID not set. Set environment variable PLAID_CLIENT_ID.",
  );
}
if (!PLAID_SECRET || PLAID_SECRET.startsWith("<REPLACE")) {
  console.warn(
    "Warning: PLAID_SECRET not set. Set environment variable PLAID_SECRET.",
  );
}
if (!DEMO_API_KEY || DEMO_API_KEY.startsWith("<REPLACE")) {
  console.warn(
    "Warning: DEMO_API_KEY not set. Set environment variable DEMO_API_KEY.",
  );
}
...
function requireDemoKey(req, res, next) {
  ...
  if (!DEMO_API_KEY || key !== DEMO_API_KEY) {
    return res
      .status(401)
      .json({ error: "unauthorized - missing or invalid demo key" });
  }
  next();
}
...
app.get("/", (req, res) => {
  res.send({
    message:
      "Plaid demo server (filesForServer). Use /create_link_token and /exchange_public_token",
...
app.get("/create_link_token", requireDemoKey, async (req, res) => {
  try {
    ...
    const resp = await axios.post(`${PLAID_BASE}/link/token/create`, body, {
      timeout: 10000,
    });
...
app.post("/create_sandbox_public_token", requireDemoKey, async (req, res) => {
  try {
    ...
    if (FAKE_PLAID) {
      ...
      console.log(
        "FAKE_PLAID: created fake public_token",
        public_token,
        "for products",
        initial_products,
      );
    ...
    if (PLAID_ENV !== "sandbox") {
      return res
        ...
        .json({
          error:
            "create_sandbox_public_token is only available in sandbox mode",
        });
    }
    ...
    const resp = await axios.post(
      `${PLAID_BASE}/sandbox/public_token/create`,
      body,
      { timeout: 10000 },
    );
    ...
    console.error(
      "create_sandbox_public_token error:",
      err?.response?.status,
      err?.response?.data || err?.message || err,
    );
    ...
    return res
      .status(status)
      .json({ error: "create_sandbox_public_token failed", details });
  }
});
...
app.post("/exchange_public_token", requireDemoKey, async (req, res) => {
  try {
    ...
    if (!public_token)
      return res.status(400).json({ error: "missing public_token in body" });
    ...
    if (FAKE_PLAID) {
      ...
      console.log(
        `FAKE_PLAID enabled: created fake access_token for item_id=${item_id}`,
      );
    ...
    const resp = await axios.post(
      `${PLAID_BASE}/item/public_token/exchange`,
      body,
      { timeout: 10000 },
    );
    ...
    if (PLAID_ENV === "sandbox" && access_token) {
      (async () => {
        try {
          console.log(
            "Sandbox auto-fire: firing INITIAL_UPDATE for access_token",
          );
          await axios.post(
            `${PLAID_BASE}/sandbox/item/fire_webhook`,
            {
              client_id: PLAID_CLIENT_ID,
              secret: PLAID_SECRET,
              access_token,
          ...
          try {
            ...
            console.log(
              "Auto-cached transactions after exchange (sandbox)",
              txResp.data?.total_transactions || 0,
            );
          } catch (fetchErr) {
            console.warn(
              "Auto-fetch transactions after sandbox webhook failed",
              fetchErr?.response?.data || fetchErr?.message || fetchErr,
            );
          }
        } catch (fireErr) {
          console.warn(
            "Auto-fire sandbox webhook failed",
            fireErr?.response?.data || fireErr?.message || fireErr,
          );
        }
      })();
    }
    ...
    console.error(
      "public_token exchange error:",
      err?.response?.data || err.message || err,
    );
    res
      .status(502)
      .json({
        error: "public_token exchange failed",
        details: err?.response?.data || err?.message,
      });
  }
});
...
app.post("/transactions_for_access_token", requireDemoKey, async (req, res) => {
  try {
    ...
    if (!access_token)
      return res.status(400).json({ error: "missing access_token in body" });

    // If FAKE_PLAID is enabled, generate or return cached fake transactions immediately.
    if (FAKE_PLAID) {
      if (!ITEM_TRANSACTIONS[access_token]) {
        ITEM_TRANSACTIONS[access_token] =
          generateFakeTransactions(access_token);
        console.log(
          `FAKE_PLAID: generated fake transactions for access_token=${access_token}`,
        );
      } else {
        console.log(
          `FAKE_PLAID: returning cached fake transactions for access_token=${access_token}`,
        );
      }
    ...
    while (attempt < maxAttempts) {
      ...
      try {
        console.log(
          `Polling transactions/sync attempt=${attempt} for access_token`,
        );
        const syncResp = await axios.post(
          `${PLAID_BASE}/transactions/sync`,
        ...
        if (
          ...
          (typeof updateStatus === "string" && updateStatus !== "NOT_READY")
        ) {
          try {
            const resp = await axios.post(
              `${PLAID_BASE}/transactions/get`,
            ...
            console.log(
              `Fetched and cached transactions after sync for access_token, total=${resp.data?.total_transactions || 0}`,
            );
            return res.json(resp.data);
          } catch (e) {
            console.warn(
              "transactions.get failed after sync indicated readiness",
      ...
      console.log(
        `transactions_for_access_token: waiting ${waitMs}ms before next sync attempt (attempt ${attempt}/${maxAttempts})`,
      );
    ...
    console.warn(
      "No transactions after polling sync; returning last known sync response",
    );
    return res
      .status(502)
      .json({
        error: "transactions fetch failed - not ready",
        details: lastSyncResp || {},
      });
  } catch (err) {
    console.error(
...
app.post(
  "/transactions_sync_for_access_token",
  requireDemoKey,
  async (req, res) => {
    try {
      ...
      const resp = await axios.post(`${PLAID_BASE}/transactions/sync`, body, {
        timeout: 20000,
      });
      return res.json(resp.data);
    } catch (err) {
      console.error(
        "transactions_sync_for_access_token error:",
        err?.response?.data || err.message || err,
      );
      res
        .status(502)
...
app.post("/webhook", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "webhook processing failed", details: err?.message });
  }
});
